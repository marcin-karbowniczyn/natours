...
var displayMap = function displayMap(locations) {
  mapboxgl.accessToken = 'pk.eyJ1IjoibWFydGlubzE5ODkiLCJhIjoiY2s0OGZlMGwzMDBydzNsdGRpc3ZiZnh0cSJ9.9gtGT9oAYiACiDqDBzc7Ug';
  ...
  var bounds = new mapboxgl.LngLatBounds(); // 1. Toworzymy markup (znacznik na mapie)
  ...
  locations.forEach(function (el) {
    ...
    new mapboxgl.Marker({
      element: html,
      anchor: 'bottom'
    }).setLngLat(el.coordinates).addTo(map); // Add popup (żeby markery pokazywały co to za lokacja)

    new mapboxgl.Popup({
      offset: 30
    }).setLngLat(el.coordinates).setHTML("<p>Day ".concat(el.day, ": ").concat(el.description, "</p>")).addTo(map);
...
var showAlert = function showAlert(type, msg) {
  ...
  document.querySelector('body').insertAdjacentHTML('afterbegin', markup); // Na samym początku body
      ...
      while (1) {
        switch (_context.prev = _context.next) {
          ...
          case 7:
            ...
            (0, _alerts.showAlert)('error', _context.t0.response.data.message); // Dokumentacja AXIOS, dlaczego takie property
      ...
      while (1) {
        switch (_context.prev = _context.next) {
          ...
          case 3:
            ...
            if (res.data.status === 'success') {
              (0, _alerts.showAlert)('success', 'Your account was created, you will be taken to main page in 3 seconds.');
              setTimeout(function () {
                location.assign('/');
              }, 3000);
            }
      ...
      while (1) {
        switch (_context.prev = _context.next) {
          case 0:
            _context.prev = 0;
            _context.next = 3;
            return _axios.default.patch("/api/v1/users/".concat(type === 'data' ? 'updateMe' : 'updateMyPassword'), data);
          ...
          case 3:
            ...
            if (response.data.status === 'success') {
              (0, _alerts.showAlert)('success', "Your user ".concat(type, " has been updated")); // setTimeout(() => {
              //   location.reload();
...
var stripe = Stripe('pk_test_Pw7AUaJHILAFBrb0fGYCkGmd00pUUB8abZ');
...
function () {
  var _ref = _asyncToGenerator(
  /*#__PURE__*/
  regeneratorRuntime.mark(function _callee(tourId, startDateId) {
    var session;
    return regeneratorRuntime.wrap(function _callee$(_context) {
      while (1) {
        switch (_context.prev = _context.next) {
          case 0:
            _context.prev = 0;
            _context.next = 3;
            return (0, _axios.default)("/api/v1/bookings/checkout-session/".concat(tourId, "/").concat(startDateId));
...
},{"axios":"../../node_modules/axios/index.js","./alerts":"alerts.js"}],"favouriteTours.js":[function(require,module,exports) {
"use strict";
...
var favouriteTours = _interopRequireWildcard(require("./favouriteTours"));
...
var favouriteBtn = document.getElementById('fav-btn');
var selectStartDate = document.querySelector('.selectStartDate'); // DELEGATION //
...
if (loginForm) loginForm.addEventListener('submit', function (e) {
  e.preventDefault();
  var email = document.getElementById('email').value; // Tak pobieraliśmy wartość inputu, lekkie przypomnienie
...
if (bookBtn) {
  bookBtn.addEventListener('click', function (e) {
    ...
    var tourId = e.target.dataset.tourId; // const tourId = e.target.dataset.tourId; // -> Bez użycia destructuring, bo dataset = { tourId }

    var startDateId = selectStartDate.value;
